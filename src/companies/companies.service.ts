...
import { 
  CreateJobDto, UpdateCompanyDto, UpdateJobDto, 
  AddTeamMemberDto, UpdateTeamMemberDto, ApplyJobDto, UpdateApplicationDto, 
  FilterJobsDto // Importe o novo DTO
} from './companies.dto';
...
export class CompaniesService {
  ...
  async findOne(id: string) {
    const company = await this.prisma.companies.findUnique({
        where: { id },
        // Retorna apenas vagas ativas na visualização da empresa
    ...
    return this.prisma.jobs.findMany({
      where: {
        ...
        OR: search ? [
          ...
          { description_md: { contains: search, mode: 'insensitive' } }
        ] : undefined,
        work_type: work_type,
        seniority: seniority,
